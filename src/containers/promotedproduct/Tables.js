...
class TablesComponent extends Component {
  ...
  handleAddUserClick(e){
    e.preventDefault()
    this.props.history.push("/adminpanel/promoted-product/add");
  }
  ...
  componentDidUpdate(prevProps) {
    if (this.props.afterRequestDelete !== prevProps.afterRequestDelete && this.props.afterRequestDelete===true) {
      const { dispatch} = this.props
      dispatch(promotedproduct_action.fetch(localStorage.getItem("token")))
      dispatch(promotedproduct_action.deleteOff())
    }
  }
  ...
  handleEditClick(id){
    ...
    //e.preventDefault()
    this.props.history.push(`/adminpanel/promoted-product/edit/${id}`);
    ...
    return (
      <div className="animated fadeIn">
        ...
        <Row style={btnStyle}>
                ...
                <Col xs="6">
                          <Button color="primary" className="px-4" onClick={this.handleAddUserClick}>New Promote</Button>
                        </Col>
            </Row>
        ...
        <Row>
          <Col>
            <Card>
              <CardHeader>
                <i className="fa fa-align-justify"></i> Promote List
              </CardHeader>
              <CardBody>
              ...
              <Table hover bordered striped responsive size="sm">
                  ...
                  { records.map((rec,i)=>(
                      <tr key={i}>
                        <td>{rec.id}</td>
                        <td>{rec.promotedProductName}</td>
                        <td>{rec.promotedProductDescription}</td>
...
function mapStateToProps(state) {
  ...
  return {
    records:state.promotedproductReducer.records,
    token:state.auth_reducer.token,
    afterRequestDelete:state.promotedproductReducer.afterRequestDelete
  }
}
